...
function getGPUProcessor() {
  if (!gpuInitPromise) {
    gpuInitPromise = (async () => {
      try {
        const webgpuModulePath = "../../../utils/webgpu-fft";
        const mod = await import(/* @vite-ignore */ webgpuModulePath);
        const WebGPUFFTProcessor = mod.default;
        ...
        // Public builds may omit the optional WebGPU FFT module.
        gpuProcessor = null;
        return false;
      }
    })();
...
export const useSpectrumCanvas = (
  ...
  }, [canvasRef, sources, streamBuffers, task, uiSettings, fftSize, setStats, renderTrigger, autoscaleOnce, interactionState, useGPU]);
};