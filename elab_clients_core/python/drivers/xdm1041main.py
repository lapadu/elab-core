"""Main driver class for OWON XDM1041 multimeter."""
try:
...
class XDM1041:
    """OWON XDM1041 multimeter serial interface class."""
    ...
    def show_available_ranges(cls):
        """Show all supported range options for each measurement mode."""
        for xdm_mode, range_dict in cls.range_ref_dict.items():
            print(f"Supported ranges for {xdm_mode.name}: ", end="")
    ...
    def __init__(self, mode: XDM1041Mode, rng: int = 0, serial_device="/dev/ttyUSB0"):
        ...
        self.logger.info("Serial port status:%s", self.serial.is_open)
    ...
    def set_range(self, rng: int) -> bool:
        ...
        if self.mode not in XDM1041.range_ref_dict:
            self.logger.error(
                "Selected mode:%s does not support range selection!",
                self.mode.name,
            )
            return False
        ...
        if rng not in range_dict.keys():
            self.logger.error("Selected range: %s is not supported!", rng)
            return False
    ...
    def test_conn(self):
        """Test connection by reading manufacturer info (IDN)."""
    ...
    def set_sample_speed_slow(self):
        """Set the sample rate to slow."""
        cmd = str(XDM1041Cmd.RATE_S)
        self.send_cmd(cmd)
        time.sleep(0.2)

    def set_sample_speed_med(self):
        """Set the sample rate to medium."""
